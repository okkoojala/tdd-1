...
function createApp(database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const cost = calculateCost(age, type,  tdate, baseCost);
    res.json({ cost });
  });
  ...
  function calculateCost(age, type, tdate, baseCost) {
    if (type === "night") {
      return calculateCostForNightTicket(age, baseCost);
    } else {
      return calculateCostForDayTicket(age, tdate, baseCost);
    }
  }
  ...
  function calculateCostForDayTicket(age,tdate, baseCost) {
    let reduction = calculateReduction(tdate);
    if (age === undefined) {
  ...
  function calculateReduction(tdate) {
    let reduction = 0;
    if (tdate && isMonday(tdate) && !isHoliday( tdate)) {
      reduction = 35;
    }
    return reduction;
  }
